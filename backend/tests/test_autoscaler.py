...
@pytest.fixture
def autoscaler():
    """AutoScaler with a fake SQS client; each test patches the per-queue depth."""
    settings = MagicMock()
...
class TestAutoScalerDecide:
    ...
    def test_depth_drives_scale_up(self, autoscaler):
        ...
        _patch_depths(autoscaler, normal=50)
        desired, snapshot = autoscaler.decide()
    ...
    def test_depth_sums_across_queues(self, autoscaler):
        ...
        _patch_depths(autoscaler, normal=10, scheduled=10, retry=5)
        desired, snapshot = autoscaler.decide()
    ...
    def test_partial_capacity_rounds_up(self, autoscaler):
        ...
        _patch_depths(autoscaler, normal=1)
        desired, snapshot = autoscaler.decide()
    ...
    def test_clamps_to_max_vms(self, autoscaler):
        ...
        _patch_depths(autoscaler, normal=10000)
        desired, _ = autoscaler.decide()
