...
# ── SQS metrics ──────────────────────────────────────────────────────────────
# Queue names come from the environment (DASS_QUEUE_NAME_*), so renaming a queue
# does not leave the exporter scraping the old name and the panels empty.
# Falls back to the current defaults when unset.
QUEUES = [
